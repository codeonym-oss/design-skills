...
- Issues live in Linear (team `COD`, project design-skills) — see `docs/agents/issue-tracker.md`.
- `main` is protected: work on a branch named after the Linear issue, open a PR with a conventional-commit title
  and `Fixes COD-<n>`; never push to `main` or edit `VERSION`/`plugin.json` versions by hand (release-please does).
